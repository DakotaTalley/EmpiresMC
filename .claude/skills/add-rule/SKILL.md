...
7. If the rule is broadly applicable beyond this project (not project-specific — i.e. it landed in `.ai/constitution.md` or a `.frameworks/<name>/rules.md`, not `.ai/project-rules.md`), mention that it can be proposed upstream for the template itself via a "Rule proposal" GitHub issue on the template repo, using its `.github/ISSUE_TEMPLATE/rule-proposal.md` template. The rule stays here locally (tagged `mutable`) either way — the issue is just how it reaches other consuming projects; `sync-guidance` already reconciles a local `mutable` row against whatever a maintainer later lands upstream.
