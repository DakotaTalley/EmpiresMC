...
8. **Publishing/discovery norms**: Successful claim/progression mods lead their pages with a one-line hook ("Expand your world, one chunk at a time!"), tag World Generation / Game Mechanics / Utility, list loader + MC-version matrix prominently, and state single-player support explicitly. ~~Permissive licensing (MIT for Chunk By Chunk/OPAC source; Apache-2.0 for Chunklock and Skyblock Builder) is the norm.~~ **Corrected 2026-08-01: this claim was wrong and should not be relied on.** Chunk By Chunk is MIT, but **Open Parties and Claims is LGPL-3.0-only** (verified on its Modrinth page), and it is the largest mod in the adjacent niche. Copyleft is well represented alongside permissive — Skyblocker (LGPL-3.0-only), GregTech CEu (LGPL-3.0), Cobblemon (MPL-2.0, ~26.5M downloads). The field is a genuine split, not a permissive consensus. Note also that license family does **not** gate modpack adoption: modpacks redistribute unmodified jars, which every OSI license permits identically. See [`licensing-decision.md`](licensing-decision.md). Mod Menu + config-screen (Cloth Config) support is an expected convenience on Fabric.
...
8. ~~**Licensing**: match the permissive norm (MIT/Apache-2.0) to encourage modpack adoption?~~ **Resolved 2026-08-01: MPL-2.0 (code) + CC BY-SA 4.0 (assets).** The question itself was malformed — there is no permissive norm (see the correction in Key Finding 8) and license family does not affect modpack adoption. See [`licensing-decision.md`](licensing-decision.md).
