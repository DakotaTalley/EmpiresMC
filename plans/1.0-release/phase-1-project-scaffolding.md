...
- **License: LGPL-3.0-or-later.** The project wants copyleft. LGPL over GPL because a Minecraft mod links against Mojang's proprietary code and is redistributed inside modpacks — LGPL keeps the mod's own source copyleft without raising GPL linking/distribution questions for those cases. MPL-2.0 is the fallback if weaker file-level copyleft is ever preferred. Record the final choice in `fabric.mod.json`'s `license` field and the README.
  - **Amended during Phase 6 (2026-07-31): relicensed to MIT.** The [market research](../research/market_research.md) found permissive licensing to be the modpack-adoption norm across every comparable mod (Chunk By Chunk and Open Parties and Claims are MIT; Chunklock and Skyblock Builder are Apache-2.0), and modpack reach is the project's growth lever. This knowingly reverses the copyleft goal stated above — the original reasoning is left intact rather than rewritten so the trade-off stays legible. MIT over Apache-2.0 because there is no patent surface here to justify the extra NOTICE ceremony. Executed as a [Phase 6](phase-6-claim-visualization.md) task.
  - **Superseded 2026-08-01: relicensed to MPL-2.0 (code) + CC BY-SA 4.0 (assets).** Full reasoning in [`licensing-decision.md`](../research/licensing-decision.md). The MIT amendment above was never executed and rested on two failed premises: (a) Open Parties and Claims is **LGPL-3.0-only**, not MIT — verified on Modrinth 2026-08-01, so half the "permissive is the norm" evidence was inverted; (b) modpacks redistribute unmodified jars, which every OSI license permits identically, so there is no adoption delta between MIT, MPL, and LGPL at all. With both premises gone, this line's original copyleft goal is reinstated — landing on the **MPL-2.0 fallback this same line already named**, which drops LGPL's library-oriented linking machinery that a gameplay mod structurally never uses.
- **No CLA or DCO — implicit inbound=outbound licensing.** Contributions are accepted under the project's current license (MPL-2.0 as of 2026-08-01; LGPL-3.0-or-later when this was written) with no separate rights grant to the maintainer. A CLA would ease future relicensing but also hands the maintainer unilateral power to relicense toward something more restrictive later (the mechanism behind MongoDB/Elastic/HashiCorp/Redis moving off open-source licenses) — that trade-off runs against the project's copyleft goal, so it's skipped. Cost: relicensing after external contributions land requires tracking down every contributor for consent.
...
- [x] Add `LICENSE` (LGPL-3.0-or-later full text), `license` field in `fabric.mod.json`, license section in README.
  ...
  - **Superseded 2026-08-01 (see amendment above):** `LICENSE` now carries the canonical MPL-2.0 body (`gh api /licenses/mpl-2.0`, SHA-verified), `fabric.mod.json` reads `"license": "MPL-2.0"`, and every `.kt` file carries the MPL Exhibit A header. The headers are not optional decoration — MPL §1.4 defines Covered Software by the attached notice, so unlike LGPL/MIT a `LICENSE` file alone would not establish file-level coverage.
